...
namespace ControleFinanceiro.Presentation.Controllers
{
    ...
    public class CategoriasController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status409Conflict)]
        public async Task<ActionResult<CategoriaDto>> Update(Guid id, [FromBody] UpdateCategoriaDto updateDto)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                var categoria = await _categoriaService.UpdateAsync(id, updateDto);
                return Ok(categoria);
            }
